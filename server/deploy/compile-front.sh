...
mkdir -p ${bak_dir}
cd $bak_dir
re_js='s/\(\/static\/js\/[a-z_A-Z0-9]*\.js\)/\/\/s.charpty.com\/\1/g'
re_css='s/\(\/static\/css\/[a-z_A-Z0-9]*\.css\)/\/\/s.charpty.com\/s\/\1/g'
re_manifest_prefix='s/\(n\.p=\"\)\/\"/\1\/\/s.charpty.com\/\"/'
# em...can not use '|' in osx like: xxxxx\(png|jpg\)xxxx
re_png='s/\/static\/image\/\([a-z_A-Z0-9]*\.png\)/\/\/s.charpty.com\/s\/\1/g'
re_jpg='s/\/static\/image\/\([a-z_A-Z0-9]*\.jpg\)/\/\/s.charpty.com\/s\/\1/g'
