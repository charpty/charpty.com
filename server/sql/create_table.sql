...
CREATE TABLE ARTICLE_META (
  ...
  `TAG` varchar(30) NOT NULL COMMENT '文章标签，可多个，空格分开',
  `SUMMARY` varchar(300) NOT NULL COMMENT '文章摘要',
  `COVER_IMAGE` varchar(100) NULL COMMENT '封面图片地址',
  `GROUP_NAME` varchar(30) NOT NULL COMMENT '组别名称',
  ...
  `PINGED` int NOT NULL DEFAULT 0 COMMENT '文章的点击量',
  `PRAISED` int NOT NULL DEFAULT 0 COMMENT '文章的点赞数量',
  `WORD_COUNT` int NOT NULL DEFAULT 0 COMMENT '文章字数,内容过长时不便统计，-1表示在程序中统计',
  `COMMENT_STATUS` tinyint(4) unsigned NOT NULL DEFAULT 20 COMMENT '评论状态，0：评论关闭且不显示已有评论，10：可评论但不显示其他评论 20：可评论并显示所有评论',
  `COMMENT_COUNT` int(11) signed NOT NULL DEFAULT -1 COMMENT '文章评论数量统计',
...
-- 长文章列表
DROP TABLE IF EXISTS ARTICLE_CONTENT_LONG;
CREATE TABLE ARTICLE_CONTENT_LONG (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `NAME` varchar(60) NOT NULL COMMENT '文章名称唯一标识，用于防止数据迁移',
  `CONTENT` mediumtext NOT NULL COMMENT '长文章内容',
  PRIMARY KEY (`ID`),
  UNIQUE KEY `NAME_UNIQUE` (`NAME`)
) ENGINE=InnoDB AUTO_INCREMENT=1000 DEFAULT CHARSET=utf8;
